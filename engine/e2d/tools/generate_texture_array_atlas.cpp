...
int main(int argc, char* argv[]) {
    ...
    parser.add_argument("--tile-width").required().scan<'i', int>().default_value(16).help("tile width").metavar("TILE_WIDTH");
    parser.add_argument("--tile-height").required().scan<'i', int>().default_value(16).help("tile height").metavar("TILE_HEIGHT");
    auto& format_options = parser.add_argument("-f", "--format").scan<'i', std::uint32_t>().required().help("image format").metavar("VK_FORMAT");
    ...
    OIIO::ImageBuf buffer = OIIO::ImageBuf(input->spec(), pixels.data());
    ...
    for (int y = 0; y < options.tiled_dimensions().y(); y++) {
        for (int x = 0; x < options.tiled_dimensions().x(); x++) {
            int tile_start_x = x * options.tile_size.x();
            int tile_start_y = y * options.tile_size.y();
            int tile_end_x = tile_start_x + options.tile_size.x();
            int tile_end_y = tile_start_y + options.tile_size.y();
            OIIO::ImageBuf cropped_image =  OIIO::ImageBufAlgo::crop(buffer, OIIO::ROI(tile_start_x, tile_end_x, tile_start_y, tile_end_y));
            if (cropped_image.has_error()) {
                std::cerr << "Cropping error: " << std::boolalpha << cropped_image.has_error() << std::endl;
                return 1;
            }
            std::vector<unsigned char> cropped_pixels(cropped_image.spec().width * cropped_image.spec().height*cropped_image.spec().pixel_bytes());
            cropped_image.get_pixels(cropped_image.roi(), input->spec().format, cropped_pixels.data());
            ktxTexture_SetImageFromMemory(ktxTexture(texture), 0, layer, KTX_FACESLICE_WHOLE_LEVEL, cropped_pixels.data(), cropped_pixels.size());
            layer+=1;
