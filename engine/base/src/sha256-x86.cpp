...
// Static initialization of SHA-256 IV in __m256i
static const __m256i sha256_iv = _mm256_setr_epi32(
    0x6a09e667, // H0
    0xbb67ae85, // H1
    0x3c6ef372, // H2
    0xa54ff53a, // H3
...
constexpr void sha256_process_x86(__m128i state[2], std::string text)
{
    state[0] = _mm_setzero_si128();
    state[1] = _mm_setzero_si128();

    state[0] = _mm256_castsi256_si128(sha256_iv);
    state[1] = _mm256_extracti128_si256(sha256_iv, 1);
    ...
    /* Save state */
    _mm_storeu_si128(&state[0], STATE0);
    _mm_storeu_si128(&state[1], STATE1);
}
